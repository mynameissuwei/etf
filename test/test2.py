...
def MOM(etf):
    ...
    long_score = annualized_returns * r_squared
    
    # 短期动量评分 (3天，使用加权回归)
    df_short = attribute_history(etf, g.m_days1, '1d', ['close'])
    y_short = np.log(df_short['close'].values)
    n_short = len(y_short)  
    x_short = np.arange(n_short)
    weights_short = np.linspace(1, 2, n_short)  # 短期也使用线性权重
    slope_short, intercept_short = np.polyfit(x_short, y_short, 1, w=weights_short)
    annualized_returns_short = math.pow(math.exp(slope_short), 250) - 1
    residuals_short = y_short - (slope_short * x_short + intercept_short)
    weighted_residuals_short = weights_short * residuals_short**2
    r_squared_short = 1 - (np.sum(weighted_residuals_short) / np.sum(weights_short * (y_short - np.mean(y_short))**2))
    short_score = annualized_returns_short * r_squared_short
    
    # 使用sigmoid平滑化并组合
    long_score_norm = sigmoid(long_score)
    short_score_norm = sigmoid(short_score)
    
    # 参考test1.py的组合逻辑
    final_score = long_score_norm * short_score_norm
